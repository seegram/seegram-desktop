...
# GITHUB_REPOSITORY first: this tree was cloned from upstream so that setting
# it up needed no credentials for a private repository, which leaves origin
# pointing at telegramdesktop/tdesktop. Deriving the slug from it sent the
# upload there, where it got a 404 that read like a missing release.
GH_REPO_SLUG="${GITHUB_REPOSITORY:-$(git remote get-url origin \
...
# The release is created by the macOS job, which runs at the same time as this
# one. Waiting rather than failing: the two builds take comparable time and
# which finishes first is a coin toss, so a hard error here fails a perfectly
# good Linux build for losing a race it was never told it was in.
echo "==> attaching the build to release $TAG in $GH_REPO_SLUG"
WAITED=0
until gh release view "$TAG" --repo "$GH_REPO_SLUG" >/dev/null 2>&1; do
	if [ "$WAITED" -ge 1800 ]; then
		echo "[ERROR] release $TAG still does not exist after 30 minutes." >&2
		echo "        the macOS job creates it - check whether it failed." >&2
		exit 1
	fi
	[ "$WAITED" -eq 0 ] && echo "    waiting for the macOS job to create it"
	sleep 30
	WAITED=$(( WAITED + 30 ))
done
