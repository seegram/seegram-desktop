...
const entries=[]; for(const size of [16,24,32,48,64,128,256]) entries.push({size,png:await sharp(svg,{density:300}).resize(size,size).png().toBuffer()});
...
fs.writeFileSync(path.join(root,'Telegram/Resources/art/icon256.ico'),Buffer.concat([header,...entries.map(e=>e.png)]));
})();
